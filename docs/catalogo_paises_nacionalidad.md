# Catálogo de países — Nacionalidad (`ddlAPP_NATL`)
 
**Campo:** `ctl00_SiteContentPlaceHolder_FormView1_ddlAPP_NATL` — pantalla
`Personal2` ("Country/Region of Origin (Nationality)"). Mapeado en
`mapeo_ds160_1.json` a la variable `nacionalidad`.
 
212 opciones (incluye el placeholder "- Select One -"). Formato:
...
**Nota:** el `value` de `IVCO` (Côte d'Ivoire) contiene un acento grave
(`` ` ``) en el texto visible del DS-160 ("COTE D`IVOIRE"), no un apóstrofo
normal — se transcribió tal cual aparece en el HTML.