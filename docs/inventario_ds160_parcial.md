...
Extraído del formulario en vivo (sesiones de pruebas) el 2026-08-20, 2026-08-21,
2026-09-03, 2026-09-22 y 2026-09-23.
...
- **Los `rbl` son contenedores `<table>`.** Los inputs reales son `id + "_0"` (`value="Y"`)
  ...
  sobre el `id` base.
- **Meses y días: conviven TRES formatos de `value` — relevado el 2026-09-22.** El `value` no se
  deduce del texto visible: en `WorkEducation1` el dia se ve `02` y el mes `APR`, y sus `value`
  son `2` y `4`. Meses: `JAN`…`DEC` en Personal1, Relatives y Spouse; numericos en Travel,
  PreviousUSTravel, WorkEducation1 y 2; `01`…`12` en PptVisa. Dias: numericos sin cero en Travel,
  PreviousUSTravel, WorkEducation1 y 2; `01`…`31` en Relatives (madre) y Spouse. La tabla
  completa, campo por campo, esta en §8 del plan.
- **Tres radios que si disparan postback y este archivo anotaba sin PB** (corregido en el mapeo
  v2.9, visto en la prueba real del 2026-09-22): `rblOtherPersonsTravelingWithYou`,
  `rblPREV_US_TRAVEL_IND` y `rblPREV_VISA_IND`. Hasta que la pagina recarga, el bloque que
  revelan no existe en el DOM.
- **Un texto mas largo que el `maxlength` no da error de validacion: rompe la sesion.** El DS-160
  responde `AppError.aspx` y hay que recuperar la solicitud con su Application ID.
- **Caracteres rechazados al guardar:** acentos en nombres y apellidos, el punto en nombres de
  empresa (`S.A.`) y la barra en explicaciones (`B1/B2`).
...
## Pantallas completadas (19 de 19)
...
```
NODE=DeceasedSpouse  url=/GenNIV/General/complete/complete_family5.aspx?node=DeceasedSpouse
NOTA: la pantalla 19. El DS-160 la intercala entre Relatives y WorkEducation1 cuando
      ddlAPP_MARITAL_STATUS = W (viudo). Con casado aparece Spouse en su lugar; con
      soltero no aparece ninguna de las dos.
NOTA DE PROCEDENCIA: relevada a mano el 2026-09-23 durante la prueba con el fixture
      jubilado, no con el extractor. Los tipos y catalogos de abajo salen del mapeo
      v2.13; faltan los maxlength y los postback, que no se midieron.
tbxGIVEN_NAME~txt~~~~Given Names del conyuge fallecido
tbxSURNAME~txt~~~~Surnames del conyuge fallecido
ddlDOBDay~sel~~01..31~~Date of Birth (dia)   <-- con cero a la izquierda, como Spouse
ddlDOBMonth~sel~~C13_JANFEB~~Date of Birth (mes)  <-- JAN..DEC, como Spouse
tbxDOBYear~txt~~4~~Date of Birth (ano)
ddlSpouseNatDropDownList~sel~~213~~Nationality (mismo catalogo que Spouse)
tbxSpousePOBCity~txt~~~~City of Birth
ddlSpousePOBCountry~sel~~281~~Country/Region of Birth (mismo catalogo que Spouse)
cbxSPOUSE_POB_CITY_NA~chk~~~~Do Not Know (ciudad de nacimiento)
NOTA: reutiliza las mismas variables del modelo que Spouse (nombresConyuge,
      apellidosConyuge, fechaNacimientoConyuge, nacionalidadConyuge,
      ciudadNacimientoConyuge, paisNacimientoConyuge). Los ids son distintos:
      tbxGIVEN_NAME / tbxSURNAME en vez de tbxSpouseGivenName / tbxSpouseSurname.
PENDIENTE: la app nunca llego a llenarla. El formato de su fecha se declaro por
      analogia con Spouse y no esta verificado.
```
...
C13_JANFEB   mes:   1|JAN  2|FEB  3|MAR  4|APR  5|MAY  6|JUN
                     ...
                     (VALUE: NO uniforme, TRES formatos. Numerico en Travel,
                      PreviousUSTravel, WorkEducation1 y 2; JAN..DEC en Personal1,
                      Relatives y Spouse; 01..12 en PptVisa. Tabla en §8 del plan)
C32_0102     dia:   1..31  (VALUE: numerico sin cero en Travel, PreviousUSTravel y
                      WorkEducation1 y 2; 01..31 en Relatives-madre y Spouse)
...
Ninguna. Las **19** pantallas del DS-160 quedaron relevadas: las 18 originales más
`DeceasedSpouse`, que aparece solo con estado civil viudo y se relevó el 2026-09-23.
