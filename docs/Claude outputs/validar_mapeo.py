...
def revisar_condicion(campo, pid, etq, variables_pantalla, errores):
    """
    'condicion' es opcional. Si esta, debe ser {variable, valor} o {variable, valorExcepto},
    y la variable disparadora tiene que existir en la MISMA pantalla: si apunta a otra
    pantalla la app no puede evaluarla y saltaria el campo en silencio.
    """
    ...
    if var not in variables_pantalla:
        errores.append(
            (
                "condicion con disparador ausente",
                pid,
                f"{etq}: '{var}' no es variable de ningun campo de esta pantalla",
            )
        )
...
def validar(ruta):
    ...
    for pantalla in mapeo.get("pantallas", []):
        ...
        # Variables que esta pantalla produce; son las unicas que una 'condicion' puede usar.
        variables_pantalla = set()
        for campo in campos:
            if isinstance(campo.get("variable"), str):
                variables_pantalla.add(campo["variable"])
            for v in campo.get("variables", []) or []:
                if isinstance(v, str):
                    variables_pantalla.add(v)
        ...
        for i, campo in enumerate(campos):
            ...
            # 7. condicion bien formada y con disparador en la misma pantalla
            revisar_condicion(campo, pid, etq, variables_pantalla, errores)
