...
export const SeccionFamilia: React.FC<Props> = ({ form }) => {
  ...
  }, [agregarFamiliarInmediato, camposFamiliaresInmediatos.length, removerFamiliaresInmediatos, tieneFamiliaresInmediatos])

  const familiaresAlTope = camposFamiliaresInmediatos.length >= MAXIMO_FILAS_REPETIDOR
  ...
  return (
    <div className="section-card">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
        <Entrada label="Nombres del padre" required {...register('nombresPadre', { onChange: soloLetras, onBlur: recortarEspacios })} error={errors.nombresPadre?.message} />
        <Entrada label="Apellidos del padre" required {...register('apellidosPadre', { onChange: soloLetras, onBlur: recortarEspacios })} error={errors.apellidosPadre?.message} />
        ...
        <Entrada label="Nombres de la madre" required {...register('nombresMadre', { onChange: soloLetras, onBlur: recortarEspacios })} error={errors.nombresMadre?.message} />
        <Entrada label="Apellidos de la madre" required {...register('apellidosMadre', { onChange: soloLetras, onBlur: recortarEspacios })} error={errors.apellidosMadre?.message} />
          ...
          <div className="md:col-span-2 space-y-4 animate-fade-in">
            {camposFamiliaresInmediatos.map((campo, indice) => (
              <div key={campo.id} className="grid grid-cols-1 md:grid-cols-2 gap-4 rounded-lg border border-slate-100 p-4 dark:border-slate-800">
                <Entrada label="Nombres" required {...register(`familiaresInmediatosDetalle.${indice}.nombres` as const, { onChange: soloLetras, onBlur: recortarEspacios })}
                  error={errors.familiaresInmediatosDetalle?.[indice]?.nombres?.message} />
                <Entrada label="Apellidos" required {...register(`familiaresInmediatosDetalle.${indice}.apellidos` as const, { onChange: soloLetras, onBlur: recortarEspacios })}
            ...
            <button type="button" onClick={agregarOtroFamiliarInmediato} disabled={familiaresAlTope}
              className="btn-secondary disabled:opacity-40 disabled:cursor-not-allowed">
              <Plus className="w-4 h-4" /> Añadir pariente inmediato
            </button>
            {familiaresAlTope && (
              <p className="text-xs text-slate-400 dark:text-slate-500">
                El DS-160 admite hasta {MAXIMO_FILAS_REPETIDOR} parientes inmediatos.
              </p>
            )}
          </div>
        )}
